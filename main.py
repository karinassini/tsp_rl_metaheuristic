...
def test_vns_solver(graph, instance):
    solver = VNS_Solver(graph)
    tour, total_distance, exploration_time, exploitation_time = solver.vns_solve(start=0)
    print("VNS Tour:", tour)
    ...
    name_without_extension = instance.split('.')[0]
    saver = SolutionSaver(instance_name=name_without_extension, save_dir=f"outputs/solutions/{name_without_extension}/vns/")
    saver.save(tour, total_distance)      # saves as JSON
...
def main():
    ...
    test_vns_solver(graph, instance)
