...
class TSPSolver:
    ...
    def build_model(self):
        ...
        self.variables = self.model.addVars(self.dist.keys(), obj=self.dist, vtype=GRB.BINARY, name="x")
        
        # Enforce symmetry: assign alias variables for reverse directions
        self.variables.update({(j, i): self.variables[i, j] for i, j in self.variables.keys()})
    ...
    def add_constraints(self):
        ...
        self.model.addConstrs(self.variables.sum(city, '*') == 2 for city in self.capitals)
